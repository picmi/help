...
Use **Navigate Inserted :::icon crosshairs-gps:::** mode to move through the field substitutions already placed in a
contract template. This makes it easier to find each field in the contract without scanning all available chips.

Also see

* [Field groups and search text](filter-fields-by-group-label-in-a-contract-template.md)
* [Insert Fields :::icon plus-circle-outline:::](insert-fields-in-a-contract-template.md) mode

:::: explanation

## What navigate mode shows

In navigate mode, PICMI shows only the inserted chips, grouped by their section such as **Information (Individual)** or
**Listing**.
...
If a field is inserted two or more times, the chip on the right shows a numbered badge to indicate how many matching
...
::: prompt
If validation fails after copying and pasting a contract, the chips usually need to be deleted and re-added from the
